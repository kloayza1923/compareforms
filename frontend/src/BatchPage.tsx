...
export function NewComparison({ capabilities }: { capabilities: Capabilities | null }) {
  ...
  return <><PageHeading title="Nueva comparación">Reúne las dos versiones del expediente y revisa cada cambio con evidencia.</PageHeading>
    ...
    <div className="row g-4"><div className="col-xl-8"><form className="surface-card" onSubmit={submit}><StepTitle number={1} title="Datos de la revisión">Cada revisión conserva sus documentos, resultados y decisiones.</StepTitle>{error && <Notice tone="danger">{error}</Notice>}
      ...
      <div className="form-check manual-check"><input className="form-check-input" id="manual-source" type="checkbox" checked={manual} onChange={event => setManual(event.target.checked)} /><label className="form-check-label" htmlFor="manual-source"><strong>Usar carga manual del origen en lugar de Roboti</strong><small>Subiré dos PDF individuales o un ZIP por versión.</small></label></div>
      {manual && <div className="manual-source-fields"><label className="form-label" htmlFor="source-system">Procedencia del original</label><select className="form-select" id="source-system" name="source_system" defaultValue="manual_otro"><option value="manual_otro">Otra fuente · carga manual</option></select><p className="form-text">Indica la procedencia real de los documentos originales.</p></div>}
      <div className="card-actions"><span>Los documentos se cargarán en el siguiente paso.</span><button className="btn btn-primary" type="submit" disabled={busy || !manual}>{busy ? 'Creando revisión…' : 'Crear revisión y cargar documentos'} <Icon name="arrow" /></button></div>
...
export function BatchPage({ batchId }: { batchId: string }) {
  ...
  const [uploadFormat, setUploadFormat] = useState<'zip' | 'pdf'>(() => {
    try { return window.localStorage.getItem('compareforms-upload-format:' + batchId) === 'zip' ? 'zip' : 'pdf'; }
    catch { return 'pdf'; }
  });
  const keyRef = useRef<string | null>(null);
  const mutationRef = useRef(false);
  useEffect(() => {
    try { setUploadFormat(window.localStorage.getItem('compareforms-upload-format:' + batchId) === 'zip' ? 'zip' : 'pdf'); }
    catch { setUploadFormat('pdf'); }
  }, [batchId]);
  const selectUploadFormat = (format: 'zip' | 'pdf') => {
    setUploadFormat(format);
    try { window.localStorage.setItem('compareforms-upload-format:' + batchId, format); }
  ...
  }, `${/\.pdf$/i.test(file.name) ? 'PDF' : 'ZIP'} ${side === 'original' ? 'de origen' : 'modificado'} recibido. Revisa el inventario y las asociaciones.`);
  const start = async () => {
  ...
  const availableOriginals = available.filter(doc => doc.side === 'original');
  const availableModified = available.filter(doc => doc.side === 'modified');
  const selectedFormat = batch?.source_mode === 'roboti' ? 'zip' : uploadFormat;
  return <><PageHeading title={batch?.name || 'Preparar expedientes'} action={<a className="btn btn-outline-secondary" href="#/history">Ver historial</a>}><span className="period-tag">{batch?.period.replace('_', ' / ')}</span> Fuente del original: {batch?.source_system === 'dalia' ? 'DALIA' : batch?.source_system === 'roboti' ? 'Roboti' : 'Carga manual'}. Verifica ambas versiones antes de iniciar.</PageHeading>
    ...
    <section className="surface-card">
      <StepTitle number={1} title="Carga de documentos">Carga las dos versiones en PDF individual o en ZIP. Cada archivo aceptado o rechazado queda en el inventario.</StepTitle>
      {batch?.source_mode !== 'roboti' && <div className="btn-group mb-3" role="group" aria-label="Formato de carga">
        <button type="button" className={'btn ' + (uploadFormat === 'pdf' ? 'btn-primary' : 'btn-outline-primary')} aria-pressed={uploadFormat === 'pdf'} disabled={busy} onClick={() => selectUploadFormat('pdf')}>Dos PDF individuales</button>
        <button type="button" className={'btn ' + (uploadFormat === 'zip' ? 'btn-primary' : 'btn-outline-primary')} aria-pressed={uploadFormat === 'zip'} disabled={busy} onClick={() => selectUploadFormat('zip')}>ZIP por versión</button>
      </div>}
      {selectedFormat === 'pdf' && <p className="form-text mb-3">Sube un PDF de origen y un PDF modificado. Después abre ambos y confirma que corresponden al mismo paciente y atención.</p>}
      <div className="row g-4"><div className="col-md-6"><DocumentDrop key={'original-' + selectedFormat} format={selectedFormat} side="original" title="PDF de origen" subtitle="Documentos sin los ajustes de auditoría" disabled={busy || batch?.source_mode !== 'manual'} onUpload={file => upload('original', file)} /></div>
        <div className="col-md-6"><DocumentDrop key={'modified-' + selectedFormat} format={selectedFormat} side="modified" title="PDF modificado" subtitle="Documentos ajustados por auditoría" disabled={busy} onUpload={file => upload('modified', file)} /></div></div>
      {busy && <Busy text="Procesando solicitud. No cierres esta pantalla durante la carga…" />}
    </section>
    <section className="surface-card mt-4"><StepTitle number={2} title="Asociación de expedientes">Confirma que cada pareja corresponde al mismo paciente y a la misma atención.</StepTitle>
      ...
      }}><div className="col-md-6"><label className="form-label" htmlFor="pair-original">Archivo de origen</label><select className="form-select" id="pair-original" name="original_id" required defaultValue={availableOriginals.length === 1 ? availableOriginals[0].id : ''}><option value="" disabled>Seleccionar original</option>{availableOriginals.map(doc => <option key={doc.id} value={doc.id}>{doc.original_name}</option>)}</select></div><div className="col-md-6"><label className="form-label" htmlFor="pair-modified">Archivo modificado</label><select className="form-select" id="pair-modified" name="modified_id" required defaultValue={availableModified.length === 1 ? availableModified[0].id : ''}><option value="" disabled>Seleccionar modificado</option>{availableModified.map(doc => <option key={doc.id} value={doc.id}>{doc.original_name}</option>)}</select></div><div className="col-md-8"><label className="form-label" htmlFor="pair-patient">Nombre del paciente verificado</label><input className="form-control" id="pair-patient" name="patient_name" maxLength={200} required placeholder="Apellidos y nombres" /></div><div className="col-md-4 d-flex align-items-end"><button className="btn btn-outline-primary w-100" disabled={busy}>Confirmar esta pareja</button></div><p className="form-text">El nombre del archivo y su prefijo no demuestran identidad. Abre ambos PDF desde el inventario, comprueba paciente y atención, y escribe el nombre verificado.</p></form></details>}
...
function DocumentDrop({ format, side, title, subtitle, disabled, onUpload }: { format: 'zip' | 'pdf'; side: string; title: string; subtitle: string; disabled: boolean; onUpload: (file: File) => Promise<void> }) {
  const [dragging, setDragging] = useState(false);
  const [filename, setFilename] = useState('');
  const [error, setError] = useState('');
  const input = useRef<HTMLInputElement>(null);
  const label = format === 'pdf' ? 'PDF' : 'ZIP';
  const choose = (file?: File) => {
    if (!file || disabled) return;
    if (!new RegExp('\\.' + format + '$', 'i').test(file.name)) { setError('Selecciona un archivo con extensión .' + format + '.'); return; }
    setError(''); setFilename(file.name); void onUpload(file);
  };
  return <div className={'zip-drop ' + (dragging ? 'dragging' : '') + ' ' + (disabled ? 'disabled' : '')} onDragOver={event => { event.preventDefault(); if (!disabled) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); if (event.dataTransfer.files.length !== 1) { setError('Carga un ' + label + ' por vez.'); return; } choose(event.dataTransfer.files[0]); }}>
    <span className={'upload-icon upload-' + side}><Icon name="upload" size={26} /></span><h3>{title}</h3><p>{subtitle}</p><button type="button" className="btn btn-outline-primary" onClick={() => input.current?.click()} disabled={disabled}>Seleccionar {label}</button><input type="file" accept={format === 'pdf' ? '.pdf,application/pdf' : '.zip,application/zip'} ref={input} aria-label={label + ' ' + title} hidden disabled={disabled} onChange={event => { choose(event.target.files?.[0]); event.target.value = ''; }} /><small>{filename ? 'Último seleccionado: ' + filename : 'También puedes arrastrarlo aquí'}</small>{error && <p className="text-danger mt-2" role="alert">{error}</p>}
  </div>;
}
